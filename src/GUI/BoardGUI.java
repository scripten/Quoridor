...
public class BoardGUI extends JFrame implements MouseListener{
	...
	public void mouseEntered(MouseEvent e) {
		...
		if(e.getSource() instanceof SquareButton){
			...
			if(players.getCurrentPlayer().hasAvailableWalls() && !verticalWall.getUsed() && board.isValidWallPlacement(verticalWall.getRow(), verticalWall.getColumn(), WALL_TYPE.VERTICAL)) {
				// TODO make isValidWallPlacement() do this
				for (int i = 0; i < 2; i++) {
					Node<State> curState = new Node<State>(null);
					Board boardCopy = Board.clone(board);
					
					boardCopy.setVerticalWall(verticalWall.getRow(), verticalWall.getColumn());
					curState.setState(new State(boardCopy, players.getPlayer(i).getCoordinates()));
					...
					if (nextMove == null) {
						System.out.println("cannot place wall.");
						return;
					}
			...
			for (int i = 0; i < 2; i++) {
				...
				boardCopy.setHorizontalWall(horizontalWall.getRow(), horizontalWall.getColumn());
				curState.setState(new State(boardCopy, players.getPlayer(i).getCoordinates()));
	...
	public void CPUTurn() {
		...
		curState.setState(new State(board, players.getCurrentPlayer().getCoordinates()));
